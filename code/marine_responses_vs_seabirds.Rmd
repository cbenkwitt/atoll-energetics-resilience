...
#repeat coral analysis with net photosynthesis (per fragment)
```{r}

marine_sb_dat_coral_comb

#try centering and scaling predictor, can't log response because net goes below 0----
range(marine_sb_dat_coral_comb$Acr_net_PP_umol_cm2_h)
...
##add in group-level effects - Chagos only data so only need atoll----
#best with log(n input)-----
acrop_frag_sb_mod_group_log<-
  brm(
 Acr_net_PP_umol_cm2_h~log_Ninput_kgha_c + (1|Atoll),
  data = marine_sb_dat_coral_combcs, 
  iter = 3000, warmup = 1000, chains = 4, cores = 4,
   control = list(adapt_delta = 0.999, max_treedepth = 15), 
      file = "brms/acrop_frag_sb_mod_group_log") 
#worked!

print(acrop_frag_sb_mod_group_log)
pp_check(acrop_frag_sb_mod_group_log) #looks good
plot(acrop_frag_sb_mod_group_log) #looks good
hypothesis(acrop_frag_sb_mod_group_log, "log_Ninput_kgha_c>0") #1 (same as before*)
